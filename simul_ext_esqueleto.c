/* 
	...
	SE SIMULARÁ UN SISTEMA DE FICHEROS TIPO LINUX SIMPLIFICADO. NO SE USARÁ UNA PARTICIÓN DEL DISCO SINO EL FICHERO particion.bin QUE CONTENDRÁ EL SISTEMA COMPLETO.
...
//define la longitud maxima que van a poder tener los comandos introducidos 
#define LONGITUD_COMANDO 100

//Declaración de las funciones que comprobarán 
...
int main(){
	...
	/*
	Inicialización de las variables en las que se guardarán la orden introducida en consola
	por el usuario, los argumentos que necesite dicha orden en caso de hacerlo, y 
	*/
	...
	*/
	...
	//se guardan los datos de particion.bin en las structs del fichero cabeceras.h
    ...
    */
    for (;;){
		...
		*/
        ...
        //COMANDO DE CONTROL PARA COMPROBAR QUE TODO FUNCIONA CORRECTAMENTE, NO INCLUIR EN EL EJERCICIO ?????????????????????????????????????????????
         else if (strcmp(orden, "prueba") == 0){
         	LeeSuperBloque(&ext_superblock);
         	Printbytemaps(&ext_bytemaps);
         	Directorio(directorio, &ext_blq_inodos);
         	Borrar(directorio, &ext_blq_inodos, &ext_bytemaps, &ext_superblock, "HOLA.txt");
         	Copiar(directorio, &ext_blq_inodos, &ext_bytemaps, &ext_superblock, memdatos,  "BelloGal.txt", "FPrueba.txt");
         	LeeSuperBloque(&ext_superblock);
         	Printbytemaps(&ext_bytemaps);
         	Directorio(directorio, &ext_blq_inodos);
            fclose(fent);
...
int BuscaFich(EXT_ENTRADA_DIR *directorio, char *nombre){
	...
	*/
	...
	El bucle finaliza sin devolver nada en caso de que al fichero no le
...
int Renombrar(EXT_ENTRADA_DIR *directorio, EXT_BLQ_INODOS *inodos, char *nombreantiguo, char *nombrenuevo){
	...
	*/
	if (BuscaFich(directorio, nombrenuevo) != 0){
		...
		return 0; //esto no tendría que ser 1??????????????????????????????????????????????????????????????
	}
	/*
	Si el nombre que se intenta asignar no es igual que el nombre
	antiguo, se actualiza el nombre del fichero y se graban los datos
	*/
...
int Borrar(EXT_ENTRADA_DIR *directorio, EXT_BLQ_INODOS *inodos, EXT_BYTE_MAPS *ext_bytemaps, EXT_SIMPLE_SUPERBLOCK *ext_superblock, char *nombre){									//???????????????????
	...
	if (BuscaFich(directorio, nombre) != 0){	
		directorio += BuscaFich(directorio, nombre);
		while(inodos->blq_inodos[directorio->dir_inodo].i_nbloque[i++] != NULL_BLOQUE);
		ext_superblock->s_free_blocks_count += (i - 1);								//Superbloque bloques libres
...
int Copiar(EXT_ENTRADA_DIR *directorio, EXT_BLQ_INODOS *inodos, EXT_BYTE_MAPS *ext_bytemaps, EXT_SIMPLE_SUPERBLOCK *ext_superblock, EXT_DATOS *memdatos, char *nombreorigen, char *nombredestino){
	...
	*/
	...
	En caso de que el nombre sea una s tampoco se admitirá, es un error de strtok()																?????????????????????????
	*/
